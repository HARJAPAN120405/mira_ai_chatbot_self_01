...
- **Your Cart card (functional)**: When the user asks to see their cart (e.g. "show my cart"), the backend sends a `cart` SSE payload; the widget renders a **Your Cart** section card with product image, title, price, quantity controls (+ / −), and a remove (trash) button. **+** and **−** update quantity via REST (`/api/cart/add`, `/api/cart/remove-one`); the card re-renders with the updated cart. **Delete** removes all units of that product (`/api/cart/remove-all`). **Proceed to Checkout** sends the message `"Proceed to checkout"` into the chat so the AI can guide checkout. Subtotal, Shipping (FREE), and Total are shown; the card stays in sync with the session cart.
- **Checkout flow (UI)**: After **Verify & Continue** (OTP), the bot shows a user message **"Continue to address selection"** and then the **address** step: "Please select a delivery address or add a new one." with selectable address cards (2px blue border, light blue home icon badge, green checkmark when selected), **+ Add New Address** (dashed), and **Continue to Payment**. On **Continue to Payment**, the user message **"Continue to payment"** is shown and the **payment** step appears: "Choose your preferred payment method." with Cash on Delivery and Credit/Debit Card cards (selected = blue border + green checkmark), an **ORDER SUMMARY** card (Subtotal, Shipping FREE, Tax, Total in blue), and a green **Place Order – $X.XX** button. After **Place order**, the **confirmation** card (premium design) shows: glassmorphism container with gradient accent bar; success badge and **Order Confirmed!** title; order ID pill; **Products ordered** section (image left, name/qty/price right, image hover zoom); **Total** + **Payment** two-column summary with payment badge; **Shipping Address** and **Delivery Status** with 📍/🚚 labels; status message in a light blue info box; and a **delivery progress** bar (Order Placed → Processing → Shipped → Delivered). Card uses Inter typography, #2563EB/#4F46E5 accent, hover lift and shadow. Classes: `.order-card`, `.order-header`, `.order-product`, `.order-summary`, `.order-shipping`, `.order-status`. When `apiBaseUrl` is empty, the widget uses a built-in Figma flow with client-side cart; cart is cleared on place order and a snapshot is passed to the confirmation step so ordered products are listed.
...
**Cart REST API (widget +/- and delete):** The server exposes REST endpoints so the cart card can update quantity and remove items without sending a chat message: `GET /api/cart?sessionId=...` (returns `{ items, total }`), `POST /api/cart/add` (`{ sessionId, title, size? }`), `POST /api/cart/remove-one` (`{ sessionId, title }`), `POST /api/cart/remove-all` (`{ sessionId, title }`). The widget calls these from the cart card callbacks and then re-renders the card with the updated cart.
...
- **Cart view**: For "show my cart" only the **Your Cart** card is shown (no duplicate table). The backend `viewCart` returns a short instruction so the agent does not echo a table; the widget renders `createYourCartCard(payload, { onCheckout, onUpdateQty, onRemove })`. **+** / **−** call the cart REST API and then replace the card with the updated cart; **Delete** (trash) calls remove-all and re-renders; **Proceed to Checkout** sends the message `"Proceed to checkout"` into the chat. Card styling: 16px radius, stronger shadow, checkout CTA with gradient button.
